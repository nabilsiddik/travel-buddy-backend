...
const userLogin = async (payload) => {
    ...
    const accessToken = generateJwtToken({ email: existingUser?.email, role: existingUser?.role, id: existingUser.id }, envVars.JWT.JWT_ACCESS_SECRET, '1h');
    // Generate refresh Token
    const refreshToken = generateJwtToken({ email: existingUser?.email, role: existingUser?.role, id: existingUser.id }, envVars.JWT.JWT_REFRESH_SECRET, '30d');
    return {
        accessToken,
        refreshToken,
    };
